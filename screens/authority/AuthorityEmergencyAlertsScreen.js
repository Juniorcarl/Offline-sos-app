...
const ALERT_SOUNDS = [
  { id: '1', label: '🚨 Siren Alert', file: require('../../assets/sounds/fire-alert.mp3') },
...
export default function AuthorityEmergencyAlertsScreen() {
  const navigation = useNavigation();
  const { fontSize, darkMode, alertSound, setAlertSound, alertVibration, setAlertVibration, alertFlashlight, setAlertFlashlight, alertRepeat, setAlertRepeat, alertVolume, setAlertVolume, alertSoundId, setAlertSoundId } = useUser();
  ...
  const [overrideSilent, setOverrideSilent] = useState(true);
    ...
    <View style={[styles.container, { backgroundColor: bg }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { fontSize: 20 * fontSize, color: textColor }]}>Incident Alerts</Text>
          <View style={{ width: 24 }} />
        </View>

        <SectionCard title="SOUND">
          <RowToggle icon="volume-high-outline" label="Sound" sublabel="Play audio on new incident" value={alertSound} onValueChange={setAlertSound} />
          <RowPress icon="musical-notes-outline" label="Alert Sound" sublabel="Choose incident alert tone" value={selectedSound.label} onPress={() => setSoundModalVisible(true)} />
          <View style={[styles.row, { borderBottomWidth: 1, borderBottomColor: borderColor, backgroundColor: cardBg, flexDirection: 'column', alignItems: 'flex-start', paddingBottom: 8 }]}>
            ...
            <Slider style={styles.slider} minimumValue={0} maximumValue={1} value={alertVolume} onValueChange={setAlertVolume} onSlidingComplete={playVolumePreview} minimumTrackTintColor="#d64045" maximumTrackTintColor={darkMode ? '#444' : '#ddd'} thumbTintColor="#d64045" />
          </View>
          <RowToggle icon="repeat-outline" label="Repeat Alert" sublabel="Keep alerting until acknowledged" value={alertRepeat} onValueChange={setAlertRepeat} last />
        </SectionCard>

        <SectionCard title="HAPTICS">
          <RowToggle icon="phone-portrait-outline" label="Vibration" sublabel="Vibrate on new incident" value={alertVibration} onValueChange={handleVibrationToggle} last />
        </SectionCard>

        <SectionCard title="VISUAL">
          <RowToggle icon="flashlight-outline" label="Flashlight" sublabel="Flash torch on new incident" value={alertFlashlight} onValueChange={setAlertFlashlight} last />
        </SectionCard>

        <SectionCard title="AUTHORITY BEHAVIOUR">
          <RowToggle icon="notifications-off-outline" label="Override Silent Mode" sublabel="Alert even when phone is on silent" value={overrideSilent} onValueChange={setOverrideSilent} />
          <RowToggle icon="phone-portrait-outline" label="Wake Screen" sublabel="Turn on screen when incident arrives" value={wakeScreen} onValueChange={setWakeScreen} />
          <RowToggle icon="flag-outline" label="High Priority Only" sublabel="Only alert for high priority incidents" value={highPriorityOnly} onValueChange={setHighPriorityOnly} last />
        </SectionCard>
        ...
        <TouchableOpacity style={styles.modalOverlay} onPress={handleCloseModal} activeOpacity={1}>
          <View style={[styles.modalCard, { backgroundColor: modalBg }]}>
            ...
            {ALERT_SOUNDS.map((s) => (
              <TouchableOpacity
                key={s.id}
                style={[styles.modalOption, { borderBottomColor: borderColor }, selectedSound.id === s.id && { backgroundColor: darkMode ? '#2a1a1a' : '#fff5f5' }]}
                onPress={() => { setAlertSoundId(s.id); playPreview(s); }}
              >
                <View style={styles.modalOptionLeft}>
                  <Text style={[styles.modalOptionText, { fontSize: 15 * fontSize, color: textColor }, selectedSound.id === s.id && { color: '#d64045', fontWeight: '600' }]}>
                    {s.label}
                  </Text>
                  {playingId === s.id && (
